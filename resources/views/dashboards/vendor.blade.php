    ...
    <script>
        (function(){
                ...
                showConfirmDialog(msg, async () => {
                    try{
                        ...
                        if(body.success){ 
                            ...
                            // Update total penjualan jika status adalah confirmed
                            if(status === 'confirmed'){
            ...
            async function updateTotalSales(){
                try{
                    const res = await fetch('/vendor/total-sales');
                    if(res.ok){
                        const data = await res.json();
                        // Update the total sales display
                        const totalSalesEl = document.querySelector('.text-3xl.font-bold.text-red-600');
                        if(totalSalesEl && data.totalSales !== undefined){
                            totalSalesEl.textContent = 'Rp ' + Number(data.totalSales).toLocaleString('id-ID');
                        }
                    }
                }catch(err){
                    console.error('Failed to update total sales:', err);
                }
            }
